...
import { u as useMockApi } from "./useMockApi-CJyUoVJj.js";
...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
    ...
    default: withCtx((_, _push2, _parent2, _scopeId) => {
      if (_push2) {
        ...
        ssrRenderList(_ctx.journeySteps, (step) => {
          _push2(`<li class="flex gap-4 items-start"${_scopeId}><span class="flex-shrink-0 w-8 h-8 rounded-app bg-accent/90 text-white text-sm font-medium flex items-center justify-center"${_scopeId}>${ssrInterpolate(step.step)}</span><div class="flex-1 min-w-0"${_scopeId}><p class="font-medium text-zinc-900"${_scopeId}>${ssrInterpolate(step.title)}</p><p class="text-muted-foreground text-sm mt-0.5"${_scopeId}>${ssrInterpolate(step.desc)}</p>`);
            ...
            (openBlock(true), createBlock(Fragment, null, renderList(_ctx.journeySteps, (step) => {
              return openBlock(), createBlock("li", {
                ...
                createVNode("div", { class: "flex-1 min-w-0" }, [
                  createVNode("p", { class: "font-medium text-zinc-900" }, toDisplayString(step.title), 1),
  ...
  ssrRenderList(_ctx.stats, (stat) => {
    _push(ssrRenderComponent(_component_AppCard, {
      ...
      default: withCtx((_, _push2, _parent2, _scopeId) => {
        if (_push2) {
          _push2(`<h3 class="app-section-title"${_scopeId}>${ssrInterpolate(stat.label)}</h3><p class="text-xl font-semibold text-zinc-900 mt-1"${_scopeId}>${ssrInterpolate(stat.value)}</p>`);
          _push2(ssrRenderComponent(_component_NuxtLink, {
            to: stat.href,
          ...
          return [
            ...
            createVNode("p", { class: "text-xl font-semibold text-zinc-900 mt-1" }, toDisplayString(stat.value), 1),
            createVNode(_component_NuxtLink, {
              to: stat.href,
              class: "text-sm text-accent hover:text-accent-hover mt-2 inline-block transition-colors"
            }, {
  ...
  _push(`<!--]--></div><div class="flex flex-wrap items-center gap-4">`);
  ...
  if (_ctx.mockMode) {
    _push(`<button type="button" class="text-xs text-muted-foreground hover:text-destructive transition-colors underline"> Simulate error (next request) </button>`);
  } else {
    _push(`<!---->`);
  }
  _push(`</div></div>`);
...
//# sourceMappingURL=index-BFoamsBL.js.map
